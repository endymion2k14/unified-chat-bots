            ...
            if (subcommand === 'scene') {
                ...
                try {
                    if (action === 'start') {
                        ...
                        client.sendMessage('Invalid action. Use start or stop');
                    }
                } catch (error) {
                    client.sendMessage(`Failed to control recording: ${error.message}`);
                }
            } else if (subcommand === 'stream') {
                if (params.length < 1) {
                    client.sendMessage('Usage: !obs stream start/stop [bot_index]');
                    return;
                }
                const action = params.shift().toLowerCase();
                let botIndex = 0;
                ...
                try {
                    ...
                    client.sendMessage(`Failed to control streaming: ${error.message}`);
                }
            } else if (subcommand === 'audio') {
                if (params.length < 2) {
                    client.sendMessage('Usage: !obs audio mute/unmute <source_name> [bot_index]');
                    return;
                }
                const action = params.shift().toLowerCase();
                const sourceName = params.shift();
