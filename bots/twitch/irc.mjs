...
export class TwitchIRC extends EventEmitter {
    ...
    async say(message) {
        ...
        else { for (let part of allParts) { this.messageQueue.push(part); log.info(`Queued single message: "${part.substring(0, 50)}..."`, `${SOURCE}-${this.channel}`); } }
        this.flushQueue();
    }

    flushQueue() {
        const now = Date.now();
        if (now - this.periodStart >= 30000) { this.periodStart = now; this.messagesInPeriod = 0; log.info(`Rate limit period reset`, `${SOURCE}-${this.channel}`); }
        
        if (this.messagesInPeriod >= 20) { log.info(`Rate limit hit: ${this.messagesInPeriod}/20 messages in current period`, `${SOURCE}-${this.channel}`); setTimeout(() => this.flushQueue(), 5000); return; }

        let next = this.messageQueue.shift();
        while (next && next.length === 0 && this.messageQueue.length > 0) { next = this.messageQueue.shift(); }
        if (!next) { log.info(`Message queue is empty`, `${SOURCE}-${this.channel}`); return; }
        ...
        else { this.messageQueue.unshift(next); log.info(`Message send failed, re-queued for retry`, `${SOURCE}-${this.channel}`); setTimeout(() => this.flushQueue(), 5000); return; }
